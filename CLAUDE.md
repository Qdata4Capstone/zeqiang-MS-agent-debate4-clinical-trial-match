...
- `attacks/` — shared, pip-installable `rag_attacks` package holding two separate PoisonedRAG-style attack implementations (genuinely different algorithms, not duplicates, so they stay distinct modules): `rag_attacks.poisonedrag_medqa`'s `PoisonedRAGBlackBoxGenerator` (generate-candidate → verify-target-LLM-answers-wrong → retry loop, from `RAG_Setting`) and `rag_attacks.poisonedrag_trial` (one-shot synthetic clinical-trial-record generation, from `Retrieving_stage`). `RAG_Setting/src/medrag_repro/attacks/poisonedrag_blackbox.py` and the poison-generation functions in `Retrieving_stage/poisonrag_experiment/run_poisonrag_experiment.py` are thin adapters/re-exports over this package — do not reimplement this attack logic locally; add it here and delegate.
- `defenses/` — shared, pip-installable `rag_defenses` package holding poisoning-defense detector classes and baseline utilities: `common.BaseDetector` (shared ABC for threshold fitting), `l2_norm` (`L2NormDetector` + `l2_norm_score`/`l2_norm_scores`), `l2_distance` (`L2DistanceDetector`, from `RAG_Setting`, centroid-distance, and `l2_distance_scores`/`leave_one_out_l2_distance_scores`, from `Agent_Setting`, nearest-neighbor-distance — two different formulas, kept as distinct names, not merged), `perplexity` (`PerplexityDetector` + `PerplexityScorer` — two distinct classes computing the same core value but kept separate since no phase has proven them behaviorally interchangeable), and `defense_baselines` (`QuantileStats`/`PerplexityStats` threshold-fitting). `RAG_Setting/src/medrag_repro/defense/{common,l2_norm,l2_distance,perplexity}.py` are thin re-export adapters over this package (its `defense/drs.py` is untouched except for a one-line `BaseDetector` import from here — `drs_defense/` still owns the DRS math itself), and `Agent_Setting/ReAct/defense_baselines.py` is a thin re-export adapter too — do not reimplement this detector/baseline logic locally; add it here and delegate.
