...
This is a research repo for clinical-trial matching and retrieval-augmented generation (RAG) robustness experiments. It contains **three independent Python subprojects** (plus three small shared libraries, `drs_defense/`, `infra/`, and `attacks/`), each with its own environment, dependencies, and entry points — there is no root-level build system, package manager, or test suite tying them together. Treat each directory below as its own project when working inside it.
